...
public class ParticipantPersistenceBridge extends AbstractPersistenceBridge<Participant> {
    ...
    public Optional<Participant> findByEmail(String email) {
        ...
        return readAll().stream().filter(participant -> email.equalsIgnoreCase(participant.getEmail())).findFirst();
    }
    ...
    public boolean save(Participant participant) {
        ...
        try {
            ...
            boolean exists = participants.stream().anyMatch(p -> participant.getEmail().equalsIgnoreCase(p.getEmail()));
            if (exists) {
    ...
    public boolean deleteByEmail(String email) {
        ...
        try {
            ...
            } else { logger.warning("Participante não encontrado para remoção: " + email); }
            return removed;
        } catch (Exception e) {
    ...
    public boolean validateLogin(String email, String password) {
        ...
        if (participant.isPresent()) {
            ...
            if (valid) { logger.info("Login de participante válido: " + email); }
            else { logger.warning("Senha incorreta para participante: " + email); }
            return valid;
        } else {
